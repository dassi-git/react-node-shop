        ...
        <div className="admin-orders-page">
            ...
            <Dialog header={selectedOrder ? `הזמנה ${selectedOrder.orderNumber}` : 'פרטי הזמנה'} visible={Boolean(selectedOrder)} onHide={() => setSelectedOrder(null)} modal className="admin-order-dialog" footer={<><Button label="סגירה" outlined onClick={() => setSelectedOrder(null)} /><Button label={isUpdating ? 'שומר...' : 'שמור סטטוס'} icon="pi pi-check" onClick={saveStatus} disabled={isUpdating || !nextStatus || nextStatus === selectedOrder?.status} /></>}>
                {selectedOrder && <div className="admin-order-details">
                    ...
                    <div className="detail-section"><h3>פריטים</h3>{selectedOrder.items?.map((item, index) => <div className="order-item" key={`${selectedOrder._id}-${index}`}><span>{item.productName} × {item.quantity}{item.seasonalSnapshot?.length > 0 && <small style={{ display: 'block' }}>{item.seasonalSnapshot.map((season) => `${season.displayName}${season.status === 'premium' ? ` (+${season.priceAdjustment} ₪)` : ''}`).join(', ')}</small>}</span><strong>{currency(item.totalPrice)}</strong></div>)}</div>
                    ...
                    <div className="detail-section"><h3>היסטוריית סטטוסים</h3>{selectedOrder.statusHistory?.length ? <div className="status-history">{selectedOrder.statusHistory.map((entry, index) => <div className="status-history-entry" key={`${entry.status}-${entry.changedAt || index}`}><Tag value={statusLabels[entry.status] || entry.status} severity={statusSeverity[entry.status] || 'secondary'} /><span>{entry.changedAt ? new Date(entry.changedAt).toLocaleString('he-IL') : '-'}</span></div>)}</div> : <p>אין היסטוריית סטטוסים</p>}</div>
