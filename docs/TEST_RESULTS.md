...
1. לנקות את כל אזהרות ESLint ולהעביר `CI=true npm run build`.
2. להפעיל DB ושרת ולשמור תוצאות API עם קודי HTTP.
3. לבדוק הרשמה והתחברות עם משתמש חדש.
4. לבדוק הוספה, שינוי כמות, מחיקה וריקון סל.
5. לבדוק שליחת הזמנה מתוך סל ולוודא את פריטי ההזמנה במסד ובמסך מנהל.
6. לבדוק הצעה, אישור, דחייה ותוקף הצעה.
7. לבדוק Stripe Test ו־PayPal Sandbox.
8. להריץ בדיקות UI/UX ידניות לפי `PRODUCTION_READINESS_CHECKLIST.md`.
9. לתעד כל כשל חדש כאן לפני תיקון.
10. להריץ regression מלא אחרי כל תיקון.
